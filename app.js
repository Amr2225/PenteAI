const rects = document.querySelectorAll("g#boardgroup g");
...
const draw = (id) => {
  ...
  board[nextMove.row][nextMove.col] = 2;

  if (checkWhereToCapture(board, nextMove.row, nextMove.col, AIPiece)) {
    for (const [i, j] of checkWhereToCapture(board, nextMove.row, nextMove.col, AIPiece)) {
      board[i][j] = 0;
      const ID = i * 19 + j;
      console.log(i, j);
      const block = document.getElementById(ID).parentElement;
  ...
  if (!gameOver) {
    const newID = nextMove.row * 19 + nextMove.col;
...
const bestMove = (board) => {
  ...
  for (const { row, col } of validLocations) {
    ...
    if (score > bestScore) {
      ...
      // if (captured) {
      //   bestMove = { row: row, col: col, captured: true };
      // } else {
      //   bestMove = { row: row, col: col, captured: false };
      // }
    }
  }
...
const minimax = (board, depth, isMaxmizingPlayer, alpha, beta) => {
  ...
  if (isMaxmizingPlayer) {
    ...
    let bestMove = validLocations[Math.floor(Math.random() * validLocations.length)];
...
const getValidLocations = (board) => {
  ...
  for (let i = 0; i < BOARD_SIZE; i++) {
    for (let j = 0; j < BOARD_SIZE; j++) {
      if (board[i][j] === 0) {
        if (hasAdjancentStone(board, i, j)) {
...
const scoring = (board, currentPiece) => {
  ...
  for (let i = 0; i < BOARD_SIZE; i++) {
    for (let j = 0; j < BOARD_SIZE - 4; j++) {
      ...
      let OppentStoneCount = col.filter((piece) => piece === oppentPiece).length;
      ...
      if (stoneCount === 5) {
        ...
        score += 180;
      } else if (OppentStoneCount === 3 && stoneCount === 1 && emptySpaceCount === 1) {
        score += 100;
  ...
  for (let i = 0; i < BOARD_SIZE; i++) {
    for (let j = 0; j < BOARD_SIZE - 4; j++) {
      ...
      let OppentStoneCount = row.filter((piece) => piece === oppentPiece).length;
  ...
  for (let i = 0; i < BOARD_SIZE - 4; i++) {
    for (let j = 0; j < BOARD_SIZE - 4; j++) {
      ...
      let OppentStoneCount = subset.filter((piece) => piece === oppentPiece).length;
